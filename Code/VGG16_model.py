...
model_vgg = VGG16_model('vgg16')
criterion = nn.CrossEntropyLoss()
optimizer = torch.optim.SGD(model_vgg.parameters(), lr=0.0001, momentum=0.9)
...
model, history = train(model_vgg,
                       x_train,
                       y_train,
                       x_val,
                       y_val,
                       save_file_name = 'model_vgg_1.pt',
                       n_epochs = N_EPOCHS,
                       BATCH_SIZE = 5)
...
plt.title('VGG-16 Training and Validation Accuracy', pad = 20)
...
plt.title('VGG-16 Training and Validation Loss', pad = 20)
...
y_actual, y_predict = predict(model, "model_vgg_1.pt", unbalanced_test_loader)
...
#------------------------------Balanced Dataset---------------------------------------------------
y_actual, y_predict = predict(model, "model_vgg_1.pt", balanced_test_loader)
