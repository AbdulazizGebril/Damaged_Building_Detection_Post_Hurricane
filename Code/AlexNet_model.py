...
model_alexnet = AlexNet_model('alexnet')
criterion = nn.CrossEntropyLoss()
optimizer = torch.optim.SGD(model_alexnet.parameters(), lr=0.00002, momentum=0.9)

def acc(x, y, return_labels=False):
    with torch.no_grad():
        logits = model_alexnet(x)
...
model, history = train(model_alexnet,
                       x_train,
                       y_train,
                       x_val,
                       y_val,
                       save_file_name = 'model_alexnet_1.pt',
                       n_epochs = N_EPOCHS,
                       BATCH_SIZE = 3)
...
plt.title('AlexNet Training and Validation Accuracy', pad = 20)
...
plt.title('AlexNet Training and Validation Loss', pad = 20)
...
y_actual, y_predict = predict(model, "model_alexnet_1.pt", unbalanced_test_loader)
...
#------------------------------Balanced Dataset---------------------------------------------------
y_actual, y_predict = predict(model, "model_alexnet_1.pt", balanced_test_loader)
